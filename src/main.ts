...
export function updateGreeting(name: string): void {
  ...
  if (!greetingName || !logoutBtn) return;
  ...
  logoutBtn.addEventListener('click', (e) => {
    ...
    showUserGreeting();
    void window.loadPage('home');
    e.stopPropagation();
  });
  document.addEventListener('click', function handler(e) {
    const target = e.target as Node;
    if (!userGreeting.contains(target)) {
      logoutBtn.style.display = 'none';
    }
  });
}

export function showUserGreeting(): void {
  const userGreeting = document.getElementById('user-greeting');
  if (!userGreeting) return;
...
export function setupNavigation(): void {
  ...
  if (hamburger && navbarMenu) {
    ...
    document.addEventListener('click', (e) => {
      if (window.innerWidth < 768 && navbarMenu.classList.contains('active')) {
        const target = e.target as Node;
        const isMenu = navbarMenu.contains(target);
  ...
  const userName = localStorage.getItem('userName');
  document.querySelectorAll<HTMLAnchorElement>('.nav-link-game').forEach((link) => {
    if (!userName) {
  ...
  const logo = document.querySelector<HTMLElement>('.navbar-brand');
  if (logo) {
...
export async function loadHomePage(): Promise<void> {
  const userName = localStorage.getItem('userName');
  let authBlock = '';
  if (!userName) {
    authBlock = `
  ...
  mainContent.innerHTML = `
    ...
    `;
  if (!userName) {
    const authBlockEl = document.querySelector<HTMLElement>('.auth-block');
    if (authBlockEl) {
  ...
  const catalogBtn = document.querySelector<HTMLAnchorElement>('.cta-catalog-btn');
  const game1Btn = document.querySelector<HTMLAnchorElement>('.cta-game1-btn');
  const game2Btn = document.querySelector<HTMLAnchorElement>('.cta-game2-btn');
  const recommendationsCatalogBtn = document.querySelector<HTMLAnchorElement>(
    '.recommendations-catalog-btn',
  );
  const homeAuthBtn = document.querySelector<HTMLButtonElement>('.auth-warning .btn');
  if (catalogBtn) {
    catalogBtn.addEventListener('click', function (e) {
      e.preventDefault();
      void window.loadPage('catalog');
    });
  }
  if (game1Btn) {
    game1Btn.addEventListener('click', function (e) {
      e.preventDefault();
      void window.loadPage('game1');
    });
  }
  if (game2Btn) {
    game2Btn.addEventListener('click', function (e) {
      e.preventDefault();
      void window.loadPage('game2');
    });
  }
